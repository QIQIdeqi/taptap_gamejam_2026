...
- **`onClick` 是属性不是方法**：UI 按钮的点击回调必须写成 `btn.onClick = function() ... end`（构造时传 `onClick = function()...` 也行，见 MenuSystem/NoteSystem/DialogueSystem:102），**绝不能**写 `btn:onClick(fn)`——后者把 `onClick` 当方法调用，但其值为 nil，会报 `attempt to call a nil value (method 'onClick')`。`onPointerEnter`/`onPointerLeave` 才是方法（用 `:` 调用）。2026-08-26 曾因此误用在 SceneManager(5处)+main.lua(1处)导致进入序章场景即崩溃，已全部改为属性赋值修复。⚠️ **连锁坑**：把 `:method(function() ... end)` 改成 `.prop = function() ... end` 时，必须同时删掉原函数调用末尾的闭合括号 `end)` → `end`！否则会报 `unexpected symbol near ')'`（2026-08-26 改完 onClick 后又因此连出 main.lua:185 与 SceneManager.lua:405 两处解析错误，均已修复）。判断 `end)` 是否合法：它是某方法/函数的实参闭合（如 `pcall(function()...end)`、`onPointerEnter(function()...end)`、`_ShowTutorial(...,function()...end)`）则合法；若前面是 `.onClick =` 赋值则必为多余括号。
