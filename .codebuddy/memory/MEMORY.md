...
- **UI 渲染挂载**：必须 `UI.Init({ scale = UI.Scale.DEFAULT })` + `UI.SetRoot(root)` 才会渲染；`Widget:Show()` 仅 SetVisible(true)，需 `UI.GetRoot():AddChild(widget)` 挂到渲染树；`Destroy()` 自动从 parent 移除。⚠️ **致命实战坑（2026-08-26）**：`UI.Panel(nil, ...)` 创建的 widget **不会自动成为渲染根**（`root_` 仅由 `UI.SetRoot` 设置，见 `urhox-libs/UI/Core/UI.lua:864`/`543`）。若只写 `local root = UI.Panel(nil,...)` 而**漏掉 `UI.GetRoot():AddChild(root)`**，整棵 UI 游离不渲染 → 看到 main.lua 总根纯黑背景 `{0,0,0,255}` → **整屏纯黑且无任何报错**。曾因此导致序章 office + 第一章 lobby/courtyard/corridor/crime_scene 全部黑屏（之前误以为 office 能显示，实际它一直因 `_makeItemBtn` 崩溃从未真正渲染过）。修复：`SceneManager.EnterScene` 创建 root 后加 `UI.GetRoot():AddChild(root)`（与 `OpeningSystem.lua:109-110` 完全一致写法）。排查黑屏时**优先查"是否真的 AddChild 到渲染树"**，而非先怀疑图片/数据。
